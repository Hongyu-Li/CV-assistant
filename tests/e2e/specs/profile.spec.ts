...
test.describe('Profile View', () => {
  ...
  test('should display save button', async ({ window }) => {
    await expect(window.locator('button', { hasText: 'Save Changes' })).toBeVisible()
  })
  ...
  test('should save profile successfully', async ({ window }) => {
    // Fill in some data
    const nameInput = window.locator('input[placeholder="John Doe"]')
    await nameInput.fill('E2E Test User')

    // Click save
    const saveBtn = window.locator('button', { hasText: 'Save Changes' })
    await saveBtn.click()

    // Check for success toast
    await expect(window.locator('text=Profile saved successfully')).toBeVisible({ timeout: 5000 })
  })
})
